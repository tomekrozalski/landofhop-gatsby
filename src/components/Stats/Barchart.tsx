...
const Barchart: React.FC<Props> = ({ positionX, positionY, sizeX, sizeY }) => {
  ...
  useEffect(() => {
    ...
    const axisX = d3.scaleBand().rangeRound([0, width]).padding(0.1);
    const axisY = d3.scaleLinear().rangeRound([height, 0]);

    const g = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`);

    d3.tsv('https://swizec.github.io/dl/barchart.tsv').then((data: DataModel[]) => {
  ...
  return <g ref={charRef} transform={`translate(${positionX}, ${positionY})`} />;
};
